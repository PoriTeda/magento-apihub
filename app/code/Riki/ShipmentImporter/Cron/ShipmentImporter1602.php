...
class ShipmentImporter1602
{
    ...
    /**
     * ShipmentImporter1602 constructor.
     * @param \Riki\ShipmentImporter\Helper\Data $dataHelper
     * @param \Magento\Framework\Stdlib\DateTime\DateTime $dateTime
     * @param DirectoryList $directoryList
     * @param Sftp $sftp
     * @param \Riki\ShipmentImporter\Logger\Logger1602 $logger
    ...
    public function __construct(
        ...
        \Riki\ShipmentImporter\Logger\Logger1602 $logger,
    ...
    {
        //1602
        $taskName = 'Delivery confirmation flow from 3PL (BIZEX     )';
        ...
        $this->_dataHelper->backupLog($needDateFile, 'shipment1602.log', '1602');
        $this->_logger->info($taskName . ' run at :' . $needDate);
        if (!$this->_dataHelper->isEnable()) {
            $this->_logger->info('Shipment importing 1602 has been disabled.');
            $this->sendMailResult();
            return;
        }
        //check sftp
        ...
        $location = $this->_dataHelper->getLocation1602();
        ...
        $path = $this->_dataHelper->getLocation1602();
            ...
            else
            {
                foreach ($content as $key => $item)
                {
                    ...
                    else
                    {
                        ...
                        $importResult = $this->processShipment1602
                        (
                            $shipKey,
                            $shipmentDate,
     ...
     * @return array
     */
    public function processShipment1602($shipKey,$shipDate, $status, $isRejected)
    {
        ...
        if($shipment->getId())
        {
            ...
            if(in_array($shipmentStatus, $allowStatusShipment))
            {
                ...
                if($isRejected)
                {
                    ...
                    (
                        $order->getStatus(),
                        __('Rejected shipment number by 1602: ' . $shipment->getIncrementId()),
                        false
                    );
                    //add History
                    $historyData = [
                ...
                else // normally
                {
                    ...
                    else
                    {
                        $this->_logger->info
                        (
                            sprintf('Shipment Status (%s) is not available to be import 1602 for shipment number: %s',$shipmentStatus,$shipKey)
                        );
                        $this->_logger->info
                        (
    ...
    public function sendMailResult()
    {
        ...
        $path = '/var/log/shipment1602.log';
        if ($reader->isExist($path)) {
            $contentLog = $reader->openFile('/var/log/shipment1602.log', 'r')->readAll();
    ...
    public function getSftpFiles($needDateFile)
    {
        ...
        $patternRoot = $this->_dataHelper->getPattern1602();
        $pattern = "/^$patternRoot/";
        ...
        if ($sfptRoot != DIRECTORY_SEPARATOR) {
            $location = $sfptRoot . $this->_dataHelper->getLocation1602();
        } else {
            $location = $this->_dataHelper->getLocation1602();
        }

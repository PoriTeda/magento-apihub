...
class ConsumerLog extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(
        ...
        \Riki\Loyalty\Model\ConsumerLogFile $consumerLog
    ) {
     ...
     */
    public function saveAPILog($apiName, $description, $status, $request, $responseData,$isSimulator = false,$error = false)
    {
        $isLogging = (int) $this->scopeConfig->getValue('riki_loyalty/api/enableLoggingApiCall',\Magento\Store\Model\ScopeInterface::SCOPE_WEBSITE);
        if(!$error && !$isLogging){
            return;
        }
        if($isSimulator){

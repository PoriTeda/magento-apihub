...
class Email extends \Magento\Framework\Model\AbstractModel
{
    ...
    protected function _getStockBlock()
    {
        if ($this->_stockBlock === null) {
            $this->_stockBlock = $this->_productAlertData->createBlock('Riki\EmailMarketing\Block\Email\Stock');
        }
    ...
    public function send()
    {
        ...
        if ($this->_type == 'price') {
            ...
            $this->sendMail($templateId,$storeId,$alertGrid);
            ...
            $textTranslate = $block->getProductStockTranslate();
            $templateId = $this->_scopeConfig->getValue(
                self::XML_PATH_EMAIL_STOCK_TEMPLATE,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
                $storeId
            ...
            foreach ($this->_stockProducts as $product) {
                $alertGrid = "";
                $alertGrid .= $newLine.$textTranslate['product_name'].": ".$product->getProductUrl();
                $alertGrid .= $newLine.$textTranslate['product_stopalert'].": "
                    .$block->getProductUnsubscribeUrl($product->getId());
                $alertGrid .= $newLine;
                $alertGrid .= $newLine.__("Unsubscribe from all stock alerts").": "
                    .$block->getUnsubscribeUrl();
     ...
     * @param $alertGrid
     */
    public function sendMail($templateId,$storeId,$alertGrid)
    {

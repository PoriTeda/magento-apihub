...
class BillingDataBuilder implements BuilderInterface
{
    ...
    public function build(array $transactions)
    {
        ...
        $companyName = '';
        $departmentName = '';
        switch ($billingAddress->getData('riki_type_address')) {
            case AddressType::HOME:
                $customerName = $billingAddress->getLastname() . ' ' . $billingAddress->getFirstname();
                break;
        ...
        $telephone = str_replace('-', '', $billingAddress->getTelephone());
        ...
        $region = $this->regionFactory->create(['resource' => $this->regionResource])
            ->load($billingAddress->getRegionId());
        if (!$region->getId()) {

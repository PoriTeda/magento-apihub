...
namespace Riki\SubscriptionPage\Block\Catalog\Product\View\Type;
...
class Bundle extends \Magento\Bundle\Block\Catalog\Product\View\Type\Bundle
{
    ...
    public function getOptions($stripSelection = false)
    {
        ...
        if (!isset($this->_optionsByProduct[$product->getId()])) {
            $this->options = null;
            $this->_optionsByProduct[$product->getId()] = parent::getOptions();
        }
    ...
    public function getOptionHtml(Option $option)
    {
        ...
        return $optionBlock->setOption($option)->setProduct($this->getProduct())->setCategoryId($this->getCategoryId())->toHtml();
    }
}

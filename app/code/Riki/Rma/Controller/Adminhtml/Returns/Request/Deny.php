...
class Deny extends \Riki\Rma\Controller\Adminhtml\Returns
{
    const ADMIN_RESOURCE = 'Riki_Rma::rma_return_actions_deny_request';
    ...
    public function execute()
    {
        ...
        foreach ($ids as $id) {
            try {
                $this->rmaManagement->denyRequest($id);
                $successCount++;
        ...
        if ($successCount) {
            $this->messageManager->addSuccess($successCount == 1
                ? __('You denied the return request successfully.')
                : __('You denied %1 return request(s) successfully.', $successCount)
            );
        }

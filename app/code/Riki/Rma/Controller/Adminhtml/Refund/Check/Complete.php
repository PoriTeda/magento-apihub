...
class Complete extends \Riki\Rma\Controller\Adminhtml\Refund
{
    ...
    const ADMIN_RESOURCE = 'Riki_Rma::rma_refund_actions_complete_check';
    ...
    public function execute()
    {
        ...
        foreach ($ids as $id) {
            try {
                $this->refundManagement->completeByCheck($id);
                $successCount++;
        ...
        if ($successCount) {
            $this->messageManager->addSuccess($successCount == 1
                ? __('You completed the refund by Check successfully.')
                : __('You completed %1 refund(s) by Check successfully.', $successCount)
            );
        }

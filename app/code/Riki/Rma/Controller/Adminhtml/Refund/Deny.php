...
class Deny extends \Riki\Rma\Controller\Adminhtml\Refund
{
    /**
     * {@inheritdoc}
     */
    public function execute()
    {
        ...
        foreach ($ids as $id) {
            try {
                $this->refundManagement->rejectWithAdj($id);
                $successCount++;
        ...
        if ($successCount) {
            $this->messageManager->addSuccess($successCount == 1
                ? __('You denied the refund successfully.')
                : __('You denied %1 refund(s) successfully.', $successCount)
            );
        }

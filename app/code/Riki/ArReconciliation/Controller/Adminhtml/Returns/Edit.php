...
use Riki\ArReconciliation\Model\OrderReturnLog;
...
class Edit extends \Magento\Backend\App\Action
{
    ...
    /*
     * @var Riki\ArReconciliation\Model\OrderReturnLog
     */
    protected  $_orderReturnLog;
    /*
     ...
     * @param OrderReturnLog $orderReturnLog
     */
    public function __construct(
        ...
        $this->_orderReturnLog = $orderReturnLog;
    ...
    public function execute()
    {
        try {
            ...
            if ($amount == "" ) {
                throw new LocalizedException(__('Please enter a return amount.'));
            }

            if (empty($date)) {
                throw new LocalizedException(__('Please enter a return date.'));
            }
            ...
            if ($order->getData('nestle_refund_amount') != $amount) {
                $generateLog = true;
                $changeAmount = true;
            }

            if ($order->getData('nestle_refund_date') != $date) {
                $generateLog = true;
                $changeDate = true;
            }
            ...
            if ($generateLog == true) {
                ...
                $order->setData('nestle_refund_amount', $amount );
                $order->setData('nestle_refund_date', $date );
            }
            ...
            $res = $this->_view->getLayout()->createBlock('Riki\ArReconciliation\Block\Adminhtml\Returns\Returns')->toHtml();
    ...
    private function generateLog( $or, $oldData, $msg, $changeType )
    {
        ...
        $model->setData( array(
            ...
            'nestle_refund_amount' => $or->getData('nestle_refund_amount'),
            'nestle_refund_date' => $or->getData('nestle_refund_date'),
            'change_type' => $changeType,
            'log' => \Zend_Json::encode( $oldData ),
            'type' => OrderReturnLog::TYPE_MANUALLY,
            'note'  => $msg,

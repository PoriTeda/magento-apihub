...
class Edit extends Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Magento_Sales::sales')
            ->addBreadcrumb(__('Time Slots'), __('Time Slots'))
            ->addBreadcrumb(__('Manage Time Slots'), __('Manage Time Slots'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This time slots not exists.'));
        ...
        $this->_coreRegistry->register('riki_timeslots', $model);

        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Time Slots') : __('New Time Slots'),
            $id ? __('Edit Time Slots') : __('New Time Slots')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Time Slots'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? __("Edit Time Slots '%1'", $model->getSlotName())  : __('New Time Slots'));

    ...
    return Component.extend({
        ...
        refreshPrice: function (item, event) {
            ...
            if (typeof event !== 'undefined') {
                ...
                var caseDisplay = 'case_display_' + keyProductId;
                var unitCase = self[caseDisplay]();
                let qtySelectedValue;

                //case
                if ('cs' == unitCase) {
                    var qtyCaseSelected = 'qty_case_selected_' + keyProductId;
                    var qtyCaseSelectedValue = self[qtyCaseSelected]();

                    var unitQtyValueKey = 'unit_qty_value_' + keyProductId;
                    var unitQtyValue = self[unitQtyValueKey]();

                    qtySelectedValue = (qtyCaseSelectedValue * unitQtyValue);
                    self['qty_selected_' + keyProductId](qtySelectedValue);
                } else {   //normal quantity
                    var qtySelected = 'qty_selected_' + keyProductId;
                    qtySelectedValue = self[qtySelected]();
                }

...
class StartConsumer
{
    ...
    public function execute()
    {
        ...
        $maxMessageConsumerCreateOrder = $this->getMaxMessageConsumerCreateOrder();

        $aCommandCreateOrder = $this->prepareListConsumerCreateOrder();

        $aCommandCreateOrder['editedSubscription'] = $this->pathBinMagento . ' queue:consumers:start --max-messages=' . $maxMessageConsumerCreateOrder . ' editedSubscription';
     ...
     * @return array
     */
    public function prepareListConsumerCreateOrder()
    {

        $sPathBinMagento = BP . '/bin/magento';

        $maxMessageConsumer = $this->getMaxMessageConsumerCreateOrder();

        $maxConsumer = $this->getMaxConsumerCreateOrder();

        /*--for subscription profile--*/
        $rangeCreateConsumer = range("a", "z");

        $aBasicCommand = [
            '%sGenerateOrderSubscription'
        ];
     ...
     * @return mixed
     */
    public function getMaxMessageConsumerCreateOrder()
    {

        return $this->scopeConfig->getValue('subscriptioncourse/createorder/number_message_per_consumer',
    ...
    {
        return $this->scopeConfig->getValue('subscriptioncourse/createorder/number_consumer_createorder',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }

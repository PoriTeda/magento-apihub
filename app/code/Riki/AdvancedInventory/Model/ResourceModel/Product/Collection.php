...
namespace Riki\AdvancedInventory\Model\ResourceModel\Product;

class Collection extends \Wyomind\AdvancedInventory\Model\ResourceModel\Product\Collection
{
    /**
    ...
    public function getStockSettings(
        ...
        $cacheKey = [$productId, $placeId, $itemId, implode(',', $placeIds), 'cacheTag' => $cacheTags];
        ...
        if ($placeId) {
            $this->getSelect()->joinLeft(
                ...
                [
                    ...
                    "default_backorderable_at_stock_level" => new \Zend_Db_Expr(
                        "IF(
                                default_use_default_setting_for_backorder=1,
                                " . $backorders . ",
                                IF(default_allow_backorder>0,1,0)
                            )"
                    ),
                    "default_managed_at_stock_level" => new \Zend_Db_Expr(
                        "IF(
                            (use_config_manage_stock=1 AND " . $manageStock . "=1) OR 
                            (use_config_manage_stock=0 AND cataloginventory_stock_item.manage_stock=1),
        ...
        if ($itemId && $placeId) {
            $this->getSelect()->joinLeft(
                ...
                'advancedinventory_assignation.item_id="' . $itemId .
                '" AND advancedinventory_assignation.place_id="' . $placeId . '"',
                [],
                null,
                'left'
            );
            $this->getSelect()->joinLeft(
                ...
                'e.entity_id=advancedinventory_stock.product_id ' . $on,
                [
                    "quantity_in_stock" => new \Zend_Db_Expr(
                        "SUM(
                            IF(
                                advancedinventory_stock.manage_stock=1,
                                advancedinventory_stock.quantity_in_stock+IFNULL(
                                    advancedinventory_assignation.qty_assigned,
                                    0
                                ),
                                0
                            )
                    ...
                    "backorder_limit_in_stock" => new \Zend_Db_Expr(
                        "SUM(
                            IF(
                                advancedinventory_stock.manage_stock=1 AND advancedinventory_stock.backorder_allowed=1,
                    ...
                    "backorder_delivery_date_allowed_in_stock" => new \Zend_Db_Expr(
                        "SUM(
                            IF(
                                advancedinventory_stock.manage_stock=1 AND 
                                advancedinventory_stock.backorder_allowed=1 AND 
                                advancedinventory_stock.place_id=1,
            ...
            $this->getSelect()->joinLeft(
                ...
                'e.entity_id=advancedinventory_stock.product_id ' . $on,
                [
                    "available_quantity"    =>  new \Zend_Db_Expr(
                        "SUM(
                            IF(
                                advancedinventory_stock.manage_stock=1,
                                advancedinventory_stock.quantity_in_stock,
                                0
                            )
                        )"
                    ),
        ...
        if (!$this->_helperData->getIndexer()) {
            foreach ($placeIds as $s) {
                $this->getSelect()->joinLeft(
                    ['advancedinventory_stock_' . $s => $advancedinventoryStock],
                    'advancedinventory_stock_' . $s .
                    ".item_id = advancedinventory_item.id AND advancedinventory_stock_" . $s . ".place_id=" . $s,
                    [
                        ...
                        "quantity_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_$s.manage_stock=1,
                                advancedinventory_stock_$s.quantity_in_stock,
                                0
                            )"
                        ),
                        "stock_id_$s" => "id",
                        "backorders_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_" . $s . ".use_config_setting_for_backorders=1,
                                " . $backorders . ",
                                IF(
                                    advancedinventory_stock_" . $s . ".backorder_allowed>0,
                                    advancedinventory_stock_" . $s . ".backorder_allowed,
                                    0
                                )
                            )"
                        ),
                        "backorders_limit_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_".$s. ".use_config_setting_for_backorders=1,
                                null,
                                IF(
                                    advancedinventory_stock_" . $s . ".backorder_allowed>0,
                                    advancedinventory_stock_" . $s . ".backorder_limit,
                                    null
                                )
                            )"
                        ),
                        "backorders_expire_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_".$s. ".use_config_setting_for_backorders=1,
                                null,
                                IF(
                                    advancedinventory_stock_" . $s . ".backorder_allowed>0,
                                    advancedinventory_stock_" . $s . ".backorder_expire,
                                    null
                                )
                            )"
                        ),
                        "backorder_delivery_date_allowed_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_".$s. ".use_config_setting_for_backorders=1,
                                null,
                                IF(
                                    advancedinventory_stock_" . $s . ".backorder_allowed>0,
                                    advancedinventory_stock_" . $s . ".backorder_delivery_date_allowed,
                                    null
                                )
                            )"
                        ),
                        "backorder_first_delivery_date_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_".$s. ".use_config_setting_for_backorders=1,
                                null,
                                IF(
                                    advancedinventory_stock_" . $s . ".backorder_allowed>0,
                                    advancedinventory_stock_" . $s . ".backorder_first_delivery_date,
                                    null
                                )
                            )"
                        ),
                        "is_in_stock_$s" => new \Zend_Db_Expr(
                            "IF(
                                advancedinventory_stock_$s.manage_stock=1,
                                IF(
                                    advancedinventory_stock_" . $s . ".quantity_in_stock>IF(
                ...
                $this->getSelect()->joinLeft(
                    ['pointofsale_' . $s => $pointofsale],
                    'pointofsale_' . $s . '.place_id =' . $s,
                    [
            ...
            foreach ($placeIds as $s) {
                ...
                $fields["default_use_default_setting_for_backorder_$s"] =
                    "default_use_default_setting_for_backorder_$s";
            }
            if (!empty($placeIds)) {
                $this->joinTable(
                    $advancedinventoryIndex,
                    $advancedinventoryIndex . '.product_id=entity_id',
                    $fields,
                    null,
                    'left'
                );
        ...
        $this->getSelect()->columns(
            [
                ...
                'backorderable_at_stock_level' => new \Zend_Db_Expr(
                    "MAX(
                        IF(
                            advancedinventory_stock.manage_stock=0,
                            1,
                            IF(
                                advancedinventory_stock.use_config_setting_for_backorders=1,
                                " . $backorders . ",
                                IF(
                                    advancedinventory_stock.backorder_allowed>0,
                                    advancedinventory_stock.backorder_allowed,
                                    0
                                )
                            )
                        )
                ...
                'backorderlimit_at_stock_level' => new \Zend_Db_Expr(
                    "IF(
                        advancedinventory_stock.manage_stock=0,
                        null,
                        IF(
                            ...
                            IF(
                                advancedinventory_stock.backorder_allowed>0,
                                IF(
                                    advancedinventory_stock.backorder_limit=0,
                                    99999999,
                                    advancedinventory_stock.backorder_limit
                                ),
                                null
                ...
                'backorderexpire_at_stock_level' => new \Zend_Db_Expr(
                    "IF(
                        advancedinventory_stock.manage_stock=0,
                        null,
                        IF(
                            ...
                            IF(
                                advancedinventory_stock.backorder_allowed>0,
                                advancedinventory_stock.backorder_expire,
                                null
                            )
                        )
                    )"
                ),
                'managed_at_product_level' => new \Zend_Db_Expr(
                    "IF(
                ...
                'managed_at_stock_level' => new \Zend_Db_Expr(
                    "IF(
                        (use_config_manage_stock=1 AND " . $manageStock . "=1) OR 
                        (use_config_manage_stock=0 AND cataloginventory_stock_item.manage_stock=1),
                        MIN(advancedinventory_stock.manage_stock),
                        0
                    )"
                ),
                'is_in_stock_at_stock_level' =>  new \Zend_Db_Expr(
                    "MAX(
                        IF(
                            advancedinventory_stock.manage_stock=1,
                            IF(
                                advancedinventory_stock.quantity_in_stock>IF(
                                                        use_config_min_qty=1," .
                                                        $minQty . ",
                                                        cataloginventory_stock_item.min_qty
                                                    ),
                ...
                'min_qty' => new \Zend_Db_Expr(
                    "IF(use_config_min_qty=1," . $minQty .
                    ",cataloginventory_stock_item.min_qty)"
                ),
            ]
        )->group("e.entity_id")->limit(1);

        // need improvement to code clear
        if ($this->registry->registry('get_stock_settings_for_update')) {
            /** @var \Riki\AdvancedInventory\Model\ResourceModel\Stock $stockResource */
            $stockResource = $this->stockFactory->create()->getResource();
            $data = $stockResource->getTransactionConnection()->fetchAll($this->getSelect());
        } else {
        ...
        $this->functionCache->store($obj, $cacheKey);

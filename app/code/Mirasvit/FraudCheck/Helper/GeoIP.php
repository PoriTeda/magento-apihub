...
class GeoIP
{
    ...
    {
        $seekCountry = $this->geoIpSeekCountry($ipnum);
        if ($seekCountry == $this->databaseSegments) {
            return null;
        }
        return $this->commonGetRecord($seekCountry);
    }
     ...
     */
    public function recordByAddr($addr)
    {
        if ($addr == null) {
            return 0;
        }
    ...
    public function geoIpSeekCountry($ipnum)
    {
        $offset = 0;
    ...
    public function commonGetRecord($seekCountry)
    {
        ...
        $recordPointer = $seekCountry + (2 * $this->recordLength - 1) * $this->databaseSegments;

        if ($this->flags & self::GEOIP_MEMORY_CACHE) {
            $recordBuf = substr($this->memoryBuffer, $recordPointer, self::FULL_RECORD_LENGTH);
        } elseif ($this->flags & self::GEOIP_SHARED_MEMORY) {
            $recordBuf = @shmop_read($this->shmid, $recordPointer, self::FULL_RECORD_LENGTH);
        } else {
            fseek($this->filehandle, $recordPointer, SEEK_SET);
            $recordBuf = fread($this->filehandle, self::FULL_RECORD_LENGTH);
        }
        $record = new \Magento\Framework\DataObject();
        $recordBufPos = 0;
        $char = ord(substr($recordBuf, $recordBufPos, 1));
        $record->setData('country_code', $this->countryCodes[$char]);
        $record->setData('country_code3', $this->countryCodesThree[$char]);
        $record->setData('country_name', $this->countryNames[$char]);
        $record->setData('continent_code', $this->continentCodes[$char]);
        $recordBufPos++;
        $strLength = 0;

        // Get region
        $char = ord(substr($recordBuf, $recordBufPos + $strLength, 1));
        while ($char != 0) {
            $strLength++;
            $char = ord(substr($recordBuf, $recordBufPos + $strLength, 1));
        }
        if ($strLength > 0) {
            $record->setData('region', substr($recordBuf, $recordBufPos, $strLength));
        }
        $recordBufPos += $strLength + 1;
        $strLength = 0;
        // Get city
        $char = ord(substr($recordBuf, $recordBufPos + $strLength, 1));
        while ($char != 0) {
        ...
        if ($strLength > 0) {
            $record->setData('city', substr($recordBuf, $recordBufPos, $strLength));
        }
        $recordBufPos += $strLength + 1;
        $strLength = 0;
        // Get postal code
        ...
        if ($strLength > 0) {
            $record->setData('postal_code', substr($recordBuf, $recordBufPos, $strLength));
        }
        $recordBufPos += $strLength + 1;
        ...
        $record->setData('latitude', ($latitude / 10000) - 180);
        for ($j = 0; $j < 3; ++$j) {
            $char = ord(substr($recordBuf, $recordBufPos++, 1));
            $longitude += ($char << ($j * 8));
        }
        $record->setData('longitude', ($longitude / 10000) - 180);
        if (self::GEOIP_CITY_EDITION_REV1 == $this->databaseType) {
            $metroareaCombo = 0;
            if ($record->getData('country_code') == "US") {
                for ($j = 0; $j < 3; ++$j) {
                    $char = ord(substr($recordBuf, $recordBufPos++, 1));
                    $metroareaCombo += ($char << ($j * 8));
                }
                $record->setData('metro_code', floor($metroareaCombo / 1000));
                $record->setData('area_code', $metroareaCombo % 1000);
            }
        }
        if ($mbExists) {
    ...
    public function safeSubstr($string, $start, $length)
    {
        // workaround php's broken substr, strpos, etc handling with
        // mbstring.func_overload and mbstring.internal_encoding
        $mbExists = extension_loaded('mbstring');

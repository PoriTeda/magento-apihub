...
define([
...
], function(jQuery, _, confirm, alert, $t){
    ...
    AdminOrder.prototype = {
        initialize : function(data){
            ...
            this.checkCedynaUrl = false;
            this.invoicedPayment = '';
            this.customerId     = data.customer_id ? data.customer_id : false;
            this.storeId        = data.store_id ? data.store_id : false;
            this.currencyId     = false;
            this.sessionUniqueKey     = data.session_unique_key ? data.session_unique_key : false;
            ...
            this.deliveryDate = data.deliveryDate ? data.deliveryDate : '';
            this.deliveryTime = data.deliveryTime ? data.deliveryTime : '';
            ...
            // fix case create order from customer view: can not not set default address
        ...
        changeAddressField : function(event){
            ...
            if (data['reset_shipping']) {
                ...
                if (name == 'country_id' || name == 'customer_address_id') {
                    this.loadArea(['shipping_method', 'billing_method', 'totals', 'items','delivery_info', 'reward_redeem'], true, data);
                }
        ...
        setShippingAsBilling : function(flag){
            var data;
            var areasToLoad = ['billing_method', 'shipping_address', 'totals', 'giftmessage','delivery_info'];
        ...
        resetShippingMethod : function(data){
            var areasToLoad = ['billing_method', 'shipping_address', 'totals', 'giftmessage', 'items', 'delivery_info', 'reward_redeem'];
            if(!this.isOnlyVirtualProduct) {
                areasToLoad.push('shipping_method');
        ...
        loadShippingRates : function(){
            ...
            this.loadArea(['shipping_method', 'delivery_info', 'totals'], true, {collect_shipping_rates: 1});
        },

        loadFakeEmails : function(){
            this.loadArea(['fakeemails'], true, {});
        },

        loadDeliveryDate : function(){
            this.isShippingMethodReseted = false;
            this.loadArea(['delivery_info', 'totals'], true, {is_show_calendar: 1});
        },
        setDeliveryDate : function(date){
            this.deliveryDate = date;
        },

        loadDeliveryInformation : function(){
            var data = {};
            data['order[collect_delivery_information]'] = 1;
            this.loadArea(['delivery_info'], true, data);
        },

        setShippingMethod : function(method){
            var data = {};
            data['order[shipping_method]'] = method;
            this.loadArea(['shipping_method', 'totals', 'billing_method', 'reward_redeem', 'delivery_info'], true, data);
        },

        setFreeSurcharge : function(){
            var data = {};

            this.loadArea([], true, data);
        },

        setEarnPoint : function(elm){
            var data = {};

            var value = jQuery(elm).is(':checked')? 1:0;

            data['set_earned_point'] = value;

            this.loadArea(['totals'], true, data);
        },

        setFreeShipping : function(elm){
            var data = {};

            var value = jQuery(elm).is(':checked')? 1:0;

            data['set_free_shipping'] = value;
            data['collect_shipping_rates'] = 1;
            this.loadArea(['shipping_method', 'reward_redeem', 'totals', 'billing_method', 'delivery_info'], true, data);
        },

        setFreeDeliveryByDeliveryGroupType : function(elm, addressId, type){
            var data = {};

            var value = jQuery(elm).is(':checked')? 1:0;

            data['set_group_type_delivery_fee'] = value;
        ...
        switchPaymentMethod : function(method){
            ...
            data['is_switch_payment_request'] = 1;
            this.loadArea(['card_validation', 'totals', 'shipping_method', 'billing_method', 'items', 'reward_redeem', 'delivery_info'], true, data);
        },
        ...
        setPaymentMethod : function(method){
            ...
            this.toggleDeliveryMessage(method);
        },
        toggleDeliveryMessage: function(paymentMethod) {
            var dDateContainer = jQuery('.field_delivery_date');
            if(paymentMethod == 'cvspayment') {
                jQuery('.admin__note_delivery').show();
                if (!jQuery('.delivery_date_picker_subscription').length) {
                    if(!jQuery('.delivery_date_back_order').length){
                        dDateContainer.find('input').val('');
                    }
                    dDateContainer.hide();
                }
            } else {
                jQuery('.admin__note_delivery').hide();
                if (!jQuery('.delivery_date_picker_subscription').length) {
                    dDateContainer.show();
                }
            }
        ...
        applyCoupon : function(code){
            this.loadArea(['items', 'shipping_method', 'totals', 'billing_method', 'reward_redeem'], true, {'order[coupon][code]':code, reset_shipping: true});
            this.orderItemChanged = false;
        },

        addProduct : function(id){
            this.loadArea(['items', 'shipping_method', 'totals', 'billing_method', 'delivery_info', 'reward_redeem'], true, {add_product:id, reset_shipping: true});
        },

        removeQuoteItem : function(id){
            this.loadArea(['items', 'shipping_method', 'totals', 'billing_method','delivery_info', 'reward_redeem'], true,
                {remove_item:id, from:'quote',reset_shipping: true});
        },

        moveQuoteItem : function(id, to){
            this.loadArea(['sidebar_'+to, 'items', 'shipping_method', 'totals', 'billing_method','delivery_info', 'reward_redeem'], this.getAreaId('items'),
        ...
        productGridRowClick : function(grid, event){
            ...
            var isSelectCaseDisplay = eventElement.tagName == 'SELECT' && eventElement.name == 'case_display';
            if (trElement && !isInputQty && !isSelectCaseDisplay) {
                var checkbox = Element.select(trElement, 'input[type="checkbox"]')[0];
                var hanpukai = jQuery(checkbox).attr('hanpukai');
                if(hanpukai && checkbox.checked){
                    return false;
                }
                ...
                if (checkbox) {
                    ...
                    if (confLink.readAttribute('disabled')) {
                        ...
                        productConfigure.setConfirmCallback(listType, function() {
                            ...
                            priceColl.innerHTML = Math.floor(productPrice) + this.currencySymbol;
            ...
            var getPriceFields = function (elms) {
                ...
                var getPrice = function (elm) {
                    ...
                    if (elm.hasAttribute('price') && !elm.disabled) {
                        return parseFloat(jQuery(elm).parents('.control.admin__field-control').find('span.price-wrapper').data('price-amount')) * optQty;
                    }
                    return 0;
                };
        ...
        productGridCheckboxCheck : function(grid, element, checked){
            if (checked) {
                if(element.inputElements) {
                    var multipleMachine = jQuery(element).attr("machine");
        ...
        productGridAddSelected : function(){
            ...
            var area = ['course','search', 'items', 'shipping_method', 'totals', 'giftmessage','billing_method','delivery_info', 'questionnaire', 'reward_redeem'];
        ...
        sidebarApplyChanges : function(auxiliaryParams) {
            if ($(this.getAreaId('sidebar'))) {
                ...
                data.reset_shipping = true;
                this.loadArea(['sidebar', 'items', 'shipping_method', 'billing_method','totals', 'giftmessage','delivery_info', 'reward_redeem'], true, data);
            }
        },
        ...
        sidebarConfigureProduct: function (listType, productId, itemId) {
            ...
            productConfigure.setOnLoadIFrameCallback(listType, function(response) {
                ...
                this.loadArea(['items', 'shipping_method', 'billing_method','totals', 'giftmessage','delivery_info', 'reward_redeem'], true);
            }.bind(this));
        ...
        itemsUpdate : function(){
            var area = ['sidebar', 'items', 'shipping_method', 'billing_method','totals', 'giftmessage','delivery_info', 'questionnaire', 'reward_redeem'];
        ...
        itemsCombine : function(){
            var area = ['sidebar', 'items', 'shipping_method', 'billing_method','totals', 'giftmessage','delivery_info', 'questionnaire', 'reward_redeem'];
            // prepare additional fields
            var fieldsPrepare = {update_items: 1,combine_item : 1};
            //fieldsPrepare['combine_item'] = 1;
            var info = $('order-items_grid').select('input', 'select', 'textarea');
            for(var i=0; i<info.length; i++){
                if(!info[i].disabled && (info[i].type != 'checkbox' || info[i].checked)) {
                    fieldsPrepare[info[i].name] = info[i].getValue();
        ...
        loadArea : function(area, indicator, params){
            ...
            if (area.indexOf('shipping_method') > -1) {
                params['order[free_shipping_fee_wbs]'] = $('order-free-shipping-fee-wbs').value
            }
        ...
        loadAreaResponseHandler : function (response) {
            if (response.unique_session_error) {
                alert({
                    content: $t('Validation failed!')
                });
                setLocation(response.unique_session_error);
            }
            if (response.error) {
                alert({
        ...
        prepareParams : function(params){
            ...
            if (!params.session_unique_key) {
                params.session_unique_key = this.sessionUniqueKey;
            }
        ...
        _realSubmit: function () {
            var disableAndSave = function(e) {
                disableElements('save');
                jQuery('#edit_form').on('invalid-form.validate', function() {

                    if(jQuery('input[name="questionnaire_show_in_admin"]:checked').val() == '1'){
                        var questionValidators = false;
                        jQuery('.question-error').remove();
                        jQuery('.level1 input[type="text"]').removeClass('mage-error');
                        jQuery('.question-container select').removeClass('mage-error');
                        jQuery('.question-container input[type="radio"]').next().removeClass('mage-error');
                        jQuery('.question-item.required').each(function() {
                            var _this = jQuery(this),
                                inputType = _this.find('.data-type').attr('rel');
                            if(inputType == '0') {
                                var checkboxName = _this.find('.level1 input[type="radio"]:first').attr('name'),
                                    checkboxElement = jQuery('input[name="'+ checkboxName +'"]'),
                                    checkboxElementChecked = jQuery('input[name="'+ checkboxName +'"]:checked'),
                                    checkboxSubElement = jQuery('select[name="'+ checkboxName.replace(/\[id\]/g,'') +'[sub]['+ checkboxElementChecked.val() +']"]'),
                                    errorHtml = '<label generated="true" class="mage-error question-error">'+ $t('Please select an option.') +'</label>';

                                if(!checkboxElementChecked.val()) {
                                    _this.append(errorHtml);
                                    questionValidators = true;
                                }
                                if(checkboxElementChecked.hasClass('has_children') && checkboxSubElement.val() == '') {
                                    checkboxSubElement.after(errorHtml);
                                    questionValidators = true;
                                }
                            }else if(inputType == '1') {
                                var selectBox = _this.find('.level1 select'),
                                    selectBoxName = selectBox.attr('name').replace(/\[id\]/g,''),
                                    subSelectBox = jQuery('select[name="'+ selectBoxName +'[sub]['+ selectBox.val() +']"]'),
                                    errorHtml = '<label generated="true" class="mage-error question-error">'+ $t('Please select an option.') +'</label>';
                                if(selectBox.val() == '') {
                                    selectBox.after(errorHtml);
                                    questionValidators = true;
                                }
                                if(subSelectBox.val() == '') {
                                    subSelectBox.after(errorHtml);
                                    questionValidators = true;
                                }
                            }else if(inputType == '2') {
                                var inputText = _this.find('.level1 input[type="text"]');
                                if(inputText.val() == '') {
                                    var errorHtml = '<label generated="true" class="mage-error question-error">'+ $t('This is a required field.') +'</label>';
                                    inputText.after(errorHtml);
                                    questionValidators = true;
                                }
                            }
                        });
                ...
                if(!form.valid()) {
                    ...
                    if(typeof window.seasonalSkipData !== 'undefined') {
                        jQuery('input.delivery_date_picker_subscription').each(function() {
                            var deliveryDate = jQuery(this).val(),
                                deliveryCode = jQuery(this).siblings('.delivery_date_code').val();
                            var boundData = _.filter(window.seasonalSkipData, function(item){
                                return item.code == deliveryCode;
                            });
                            if(boundData.length) {
                                jQuery(boundData[0].allow_skip_date).each(function() {
                                    var item = jQuery(this)[0];
                                    var deliveryDateSeleted = new Date(deliveryDate),
                                        startDate = new Date(item.allow_skip_from),
                                        endDate = new Date(item.allow_skip_to);
                                    deliveryDateSeleted.setHours(0,0,0,0);
                                    if(deliveryDateSeleted >= startDate && deliveryDateSeleted <= endDate) {
                                        canCheckout = false;
                                        alert({
                                            content: $t("Selected Seasonal Limited items can not be delivered at the specified delivery date and time, so please change the desired delivery date.")
                                        });
                                        return false;
                                    }
                                });
                    ...
                    if( paymentMethod == e.invoicedPayment && e.customerId ){
                        jQuery.ajax({
                            type : 'post',
                            url : e.checkCedynaUrl,
                            dataType: 'json',
                            data: {
                                form_key: FORM_KEY,

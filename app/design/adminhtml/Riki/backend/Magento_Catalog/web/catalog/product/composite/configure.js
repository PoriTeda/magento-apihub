...
define([
    ...
    ProductConfigure.prototype = {
        ...
        /**
         * Immediatly apply options data get it through ajax
         *
         * @param listType scope name
         * @param itemId
         */
        applyItemConfiguration: function (listType, itemId, qtyElement, grid, priceColl, checkbox, currencySymbol, productBasePrice) {
            if (!listType || !itemId ) {
                return false;
            }
            this.initialize();
            ...
            this._requestItemConfigurationData(listType, itemId, qtyElement, grid, priceColl, checkbox, currencySymbol, productBasePrice);
        },

        /**
         * Get configuration data of product through ajax and show them
         *
         * @param listType scope name
         * @param itemId
         */
        _requestItemConfigurationData: function (listType, itemId, qtyElement, grid, priceColl, checkbox, currencySymbol, productBasePrice) {
                    ...
                    onSuccess: function (transport) {
                        ...
                        if (response.isJSON()) {
                            ...
                            if (!$(confirmedBlockId)) {
                                ...
                                $(confirmedBlockId).update();
                            }
                            $(confirmedBlockId).insert(response);


                            var elms = $(confirmedBlockId).getElementsByTagName('input');

                            for (var i = 0; i < elms.length; i++) {
                                if (elms[i].name == 'qty') {
                                    var confirmedCurrentQty =  elms[i];
                                }
                            }

                            if (qtyElement && confirmedCurrentQty && !isNaN(confirmedCurrentQty.value)) {
                                confirmedCurrentQty.value = qtyElement.value;
                            }
                            // calc and set product price
                            var productPrice = this._calcProductPrice(confirmedBlockId);
                            if (this._isSummarizePrice()) {
                                productPrice += productBasePrice;
                            }
                            productPrice = parseFloat(productPrice);
                            priceColl.innerHTML = Math.floor(productPrice) + currencySymbol;
        ...
        _isSummarizePrice: function(elm) {
            if (elm && elm.hasAttribute('summarizePrice')) {
                return parseInt(elm.readAttribute('summarizePrice'));
            }
            return true;
        },
         ...
         */
        _calcProductPrice: function (confirmedBlockId) {
            var productPrice = 0;
            var getPriceFields = function (elms) {
                ...
                var getPrice = function (elm) {
                    ...
                    if (elm.hasAttribute('price') && !elm.disabled) {
                        var price = parseFloat(jQuery(elm).parents('.control.admin__field-control').find('span.price-wrapper').data('price-amount'))
                        return  price * optQty;
                    }
                    return 0;
                };
            ...
            productPrice += getPriceFields($(confirmedBlockId).getElementsByTagName('input'));
            productPrice += getPriceFields($(confirmedBlockId).getElementsByTagName('select'));
            productPrice += getPriceFields($(confirmedBlockId).getElementsByTagName('textarea'));
            return productPrice;
        },

        ...
        foreach ($data as $key => $value) {
            if (($key === 'conditions' || $key === 'actions') && is_array($value)) {
                foreach ($value as $id => $data) {
                    ...
                    /**
                     * NED-3942 (remove space from condition for is one of & is not one of)
                     */
                    $isContainsOperator = !empty($data['operator']) && in_array($data['operator'], ['()', '!()']);
                    if (!empty($data['operator']) && $isContainsOperator) {
                        $tmp = [];
                        if (is_string($data['value']) && false !== strpos($data['value'], ',')) {
                            foreach (explode(',', $data['value']) as $value) {
                                $tmp[] = trim($value);
                            }
                            $data['value'] = implode(',', $tmp);
                        } elseif (is_array($data['value'])) {
                            foreach ($data['value'] as $tmpValue){
                                $tmp[] = $tmpValue;
                            }
                            $data['value'] = $tmp;
                        }

...
class SaveAction
{
    ...
    public function save(ProductInterface $bundleProduct, OptionInterface $option)
    {
        ...
        try {
            ...
            $this->checkOptionProduct($option->getData(), $option->getProductLinks(), $e);
     ...
     */
    public function checkOptionProduct($optionData, $productLink, $exceptionMessage)
    {
        if (!empty($optionData))
        {
            $logger = new \Zend\Log\Logger();
            $logger->addWriter(new \Zend\Log\Writer\Stream(BP . '/var/log/support_report.log'));
            $logger->info('NED-6707 detail log: '.json_encode($optionData));

            if (count($productLink) > 0)
            {
                foreach ($productLink as $childData)
                {
                    $logger->info('Product Links: '.json_encode($childData->getData()));
                }
            }
            $logger->info('Exception CouldNotSaveException: '.$exceptionMessage);
        }
    }

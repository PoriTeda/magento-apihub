...
class DbStorage extends AbstractStorage
{
    ...
    {
        $this->logger->info("NED-5504: Begin deleteOldUrls");
        $oldUrlsSelect = $this->connection->select();
        $oldUrlsSelect->from(
            $this->resource->getTableName(self::TABLE_NAME)
        );
        // NED-5540 optimize the queries, adding array unique filter
        $data = [];
        foreach ($urls as $url) {
            $data[] = [UrlRewrite::ENTITY_TYPE => $url->getEntityType(), UrlRewrite::ENTITY_ID => $url->getEntityId(), UrlRewrite::STORE_ID => $url->getStoreId() ];
        }
        $dataToquery = array_unique($data, SORT_REGULAR);
        $this->logger->info("NED-5504: totals urls  - " . count($urls));
        $this->logger->info("NED-5504: totals dataToquery - " . count($dataToquery));
        /** @var UrlRewrite $url */
        foreach ($dataToquery as $element) {
            $oldUrlsSelect->orWhere(
                $this->connection->quoteIdentifier(
                    UrlRewrite::ENTITY_TYPE
                ) . ' = ?',
                $element[UrlRewrite::ENTITY_TYPE]
            );
            $oldUrlsSelect->where(
                $this->connection->quoteIdentifier(
                    UrlRewrite::ENTITY_ID
                ) . ' = ?',
                $element[UrlRewrite::ENTITY_ID]
            );
            $oldUrlsSelect->where(
                $this->connection->quoteIdentifier(
                    UrlRewrite::STORE_ID
                ) . ' = ?',
                $element[UrlRewrite::STORE_ID]
            );
        }
        ...
        if ($hasOldUrls) {
            $e = new \Exception();
            $this->logger->info("NED-5504 : Begin delete from Select" );
            $this->logger->info("NED-5504 : Trace : " . $e->getTraceAsString());
